...
/**
 * 
 * @author Game by Peter Camerili and Michael Truong
 * Coded by the clam man
 */
...
public class ProperTest extends RomaTest{
    ...
    public void run() {
        ...
        assert(r.activateCard (3, new int[0]));                 //new int[1] used to avoid
        assert(r.getAvailableActionDice().length == 2); // emtpy array issues
        assert(p2.getVictoryPoints () == 10);
        assert(r.activateCard (3, new int[0]));
        assert(r.getAvailableActionDice().length == 1);
        ...
        int numVpsToBuy = 8;
        int arg2[] = {2, numVpsToBuy}; // fixed to include bribe cost
        ...
        //activate legat
        assert(r.activateCard (5, new int[0]));
        ...
        //test2(); //uncomment this line once it is debugged
    }
    
    /**
     * Yet to be debugged, can someone else do it?
     */
    public void test2() {
        RomaEngine r = new Roma();
    ...
    public String toString(){
        return "Full Sick Badass Test v2";
    }
